"""Command-line entry for hyperfine-bench."""
...
from hyperfine_bench.parse_tests import TestsParseError, parse_tests_file
...
def _build_parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(
        prog="hyperfine-bench",
        description="Compile assembly (and optional C/C++ baselines), run IO tests, "
        "and benchmark binaries with hyperfine in one script (local or single Docker run).",
    )
    p.add_argument(
        "assembly",
        type=Path,
    ...
    p.add_argument(
        ...
        type=int,
        default=30,
        help="Hyperfine benchmark iterations per binary (default: 30).",
    )
    p.add_argument(
        "--hyperfine-warmup",
        type=int,
    ...
    p.add_argument(
        ...
        type=float,
        default=10.0,
        help="Per-execution timeout in seconds for tests and each benchmark iteration (default: 10).",
    )
    p.add_argument(
        "--timing-input",
        type=Path,
        default=None,
        help="File copied to timing.in and used when no tests are provided (default: empty stdin).",
    )
    p.add_argument(
        "--strict-output",
        action="store_true",
    ...
    p.add_argument(
        "--only-compile",
        action="store_true",
        help="Compile only; skip tests and benchmarking.",
    )
    p.add_argument(
        "--skip-compile",
        action="store_true",
    ...
    p.add_argument(
        ...
        type=float,
        default=1200.0,
        help="Wall-clock cap for the whole script subprocess (default: 1200s).",
    )
    p.add_argument(
        "--workspace",
        type=Path,
...
def _build_timing_chart_lines(report: dict, *, bar_width: int = 44) -> list[str]:
    ...
    lines: list[str] = [
        ...
        "=" * 78,
        "  TIMING CHART",
        "  Mean / median = wall time for one full benchmark iteration (hyperfine result).",
        "  vs fastest = mean / smallest mean in this run. Bar length is proportional to mean.",
        "=" * 78,
        "",
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.tests is not None:
        try:
            ...
            print(f"hyperfine-bench: invalid tests file: {exc}", file=sys.stderr)
            return 2
        except OSError as exc:
            print(f"hyperfine-bench: cannot read tests: {exc}", file=sys.stderr)
            return 2
    ...
    try:
        ...
        print(f"hyperfine-bench: {exc}", file=sys.stderr)
        return 2
